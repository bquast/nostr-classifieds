export async function onRequest(context) {
    try {
        const events = await fetchClassifiedsFromNostr();
...
async function fetchClassifiedsFromNostr() {
    const relayUrls = [
        'wss://relay.damus.io',
        'wss://nos.lol',
        'wss://relay.nostr.net'
    ...
    for (const relayUrl of relayUrls) {
        const eventsFromRelay = await connectAndFetch(relayUrl, subId);
...
function connectAndFetch(relayUrl, subId) {
    return new Promise((resolve, reject) => {
        ...
        ws.onopen = () => {
            const filter = { kinds: [30402], limit: 50 }; // Fetch more per relay, then limit overall
